...
Documentation update policy for this repository:
- Keep README/wiki aligned with actually supported features and limitations.
- When behavior changes, document validation commands used (`spotless`, `test`, optional `verify`).
- Keep examples consistent with Java 21 + Maven + Spring JDBC/SPI architecture.
- Avoid documenting unsupported custom query systems or ORM-like features.
