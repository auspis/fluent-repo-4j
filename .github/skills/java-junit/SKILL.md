...
JUnit guidance specific to this repository:

- Use JUnit 5 with AssertJ for assertions.
- Keep names compact and behavior-oriented.
- Follow the repository test pyramid rules:
  - Unit tests: no test annotation required.
  - Component tests: annotate with `@ComponentTest`.
  - Integration tests: annotate with `@IntegrationTest`.
  - E2E tests: annotate with `@E2ETest` and reserve for real database workflows.
- For DB-related behavior tests, prefer existing utility and fixtures:
  - `io.github.auspis.fluentsql4j.test.util.TestDatabaseUtil`
  - entities from `io.github.auspis.fluentrepo4j.test.domain`.
- Use fast local command as default verification: `./mvnw clean test`.
