...
Spring guidance specific to this repository:

- This repository is a library, not an application service stack.
- Prioritize Spring Data Commons SPI integration and Spring JDBC primitives.
- Do not introduce JPA runtime behavior or ORM-style persistence patterns.
- Keep mapping constrained to Jakarta annotations (`@Table`, `@Column`, `@Id`, `@GeneratedValue`) and existing mapping conventions.
- Use explicit local types (no `var`) and constructor injection.
- Reflection should be avoided by default; if unavoidable, keep it localized, documented, and tested.
- Validate changes with `./mvnw spotless:apply` and `./mvnw clean test`.
