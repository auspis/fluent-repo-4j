...
Refactoring constraints for this repository:

- Preserve current repository behavior and public APIs unless the task explicitly asks for API change.
- Enforce Java style constraints already used by the project:
  - no local `var`
  - explicit types
  - constructor injection and immutable fields where practical
- Respect class conventions:
  - Helper classes use `Helper` suffix and live in `*.helper`
  - Util classes use `Util` suffix in `*.util`, static-only, private constructor
- Keep persistence architecture within Spring Data Commons SPI + Spring JDBC scope.
- Use `./mvnw spotless:apply` and `./mvnw clean test` after non-trivial refactors.
