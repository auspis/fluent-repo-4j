...
When planning refactors in this repository:

- Keep refactors behavior-preserving and incremental.
- Include explicit guardrails in the plan:
  - no local `var`
  - keep Helper vs Util naming/package conventions
  - preserve Spring Data SPI and JDBC boundaries
  - avoid reflection unless explicitly justified
- Add a verification step after each phase:
  - `./mvnw spotless:apply`
  - `./mvnw clean test`
- For persistence-related refactors, ensure plan includes checks for ID generation behavior and exception semantics.
