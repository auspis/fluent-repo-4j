...
Test-generation behavior for this repository:

- Target Java + Maven conventions first.
- Generate tests that respect this pyramid and annotations:
  - Unit (default)
  - `@ComponentTest` for mocked JDBC collaborations
  - `@IntegrationTest` for H2-backed integration
  - `@E2ETest` only for real DB/Testcontainers scenarios
- Prefer AssertJ style assertions.
- Reuse existing domain fixtures and helpers before inventing local test entities.
- Default verification command is `./mvnw clean test`.
- Do not silently generate slow E2E tests unless explicitly requested.
